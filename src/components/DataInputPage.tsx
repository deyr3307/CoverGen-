...
export function DataInputPage({ 
  ...
  // Dedicated boolean state for Joint Evaluator toggle
  const [isJointEvaluator, setIsJointEvaluator] = useState<boolean>(() => 
    Boolean(coverData.isJointEvaluator || (coverData.teacher2Name && coverData.teacher2Name.trim().length > 0))
  );

  useEffect(() => {
    const isPresent = Boolean(coverData.isJointEvaluator || (coverData.teacher2Name && coverData.teacher2Name.trim().length > 0));
    setIsJointEvaluator(isPresent);
  }, [coverData.isJointEvaluator, coverData.teacher2Name]);

  const handleToggleJointEvaluator = () => {
    setIsJointEvaluator((prev) => {
      const nextVal = !prev;
      if (nextVal) {
        setCoverData(curr => ({
          ...curr,
          isJointEvaluator: true,
          teacher2Name: curr.teacher2Name?.trim() ? curr.teacher2Name : 'Dr. Sadia Islam Mou',
          teacher2Designation: curr.teacher2Designation?.trim() ? curr.teacher2Designation : 'Associate Professor',
          teacher2Discipline: curr.teacher2Discipline?.trim() ? curr.teacher2Discipline : (curr.teacherDiscipline || curr.departmentName || 'Environmental Science Discipline'),
          teacher2University: curr.teacher2University?.trim() ? curr.teacher2University : (curr.teacherUniversity || curr.universityName || 'Khulna University'),
          teacher2Location: curr.teacher2Location?.trim() ? curr.teacher2Location : (curr.teacherLocation || '')
        }));
      } else {
        setCoverData(curr => ({
          ...curr,
          isJointEvaluator: false,
          teacher2Name: '',
          teacher2Designation: '',
          teacher2Discipline: '',
          teacher2University: '',
      ...
      <div className="w-full max-w-[24.5rem] md:max-w-md space-y-5 relative z-10 font-sans pb-16">
        ...
        <div className={`p-5 md:p-6 rounded-2xl space-y-5 ${cardClass}`}>
          ...
          <div className="space-y-4 pt-1">
            ...
            {/* Joint Evaluator / Second Teacher Toggle & Section */}
            <div className="pt-2">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center space-x-2.5">
                  <input 
                    id="joint-evaluator-page-toggle"
                    type="checkbox"
                    checked={isJointEvaluator}
                    onChange={() => handleToggleJointEvaluator()}
                    className={`w-4 h-4 rounded focus:outline-none focus:ring-0 transition-all cursor-pointer ${
                      isDark ? 'border-slate-800 bg-[#070b13] text-rose-500' : 'border-slate-300 bg-white text-rose-600'
                    }`}
                  />
                  <label 
                    htmlFor="joint-evaluator-page-toggle"
                    className={`text-[11px] font-mono font-extrabold uppercase tracking-wider transition-all select-none cursor-pointer ${
                      isDark ? 'text-slate-300 hover:text-rose-400' : 'text-slate-700 hover:text-rose-600'
                    }`}
                  >
                    Add Joint Evaluator / Co-Supervisor
                  </label>
                </div>

                <button
                  type="button"
                  onClick={() => handleToggleJointEvaluator()}
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold transition-all cursor-pointer flex items-center gap-1.5 border ${
                    isJointEvaluator
                      ...
                      : isDark
                        ? 'bg-[#0e1424] border-[#1e2a4a] text-slate-400 hover:text-slate-200'
                        : 'bg-slate-100 border-slate-200 text-slate-600 hover:bg-slate-200'
                  }`}
                >
                  <span className={`w-2 h-2 rounded-full ${isJointEvaluator ? 'bg-white animate-pulse' : 'bg-slate-400'}`} />
                  <span>{isJointEvaluator ? 'ENABLED' : 'OFF'}</span>
                </button>
              </div>

              {isJointEvaluator && (
                <div className={`mt-3 p-4 rounded-xl space-y-3.5 border border-dashed transition-all ${
                  isDark ? 'border-rose-500/30 bg-rose-950/10' : 'border-rose-300 bg-rose-50/40'
                }`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono font-extrabold uppercase tracking-wider text-rose-500 flex items-center gap-1.5">
                      <UserCheck className="w-3.5 h-3.5 text-rose-500" />
                      Joint Evaluator Details
                    </span>
                    <button
                      type="button"
                      onClick={() => handleToggleJointEvaluator()}
                      className="text-[9px] font-mono text-rose-400 hover:text-rose-600 underline cursor-pointer flex items-center gap-1"
                    >
                  ...
                  <div className="space-y-1">
                    <label className={`block text-[9px] font-mono font-extrabold uppercase tracking-widest ${isDark ? "text-rose-400" : "text-rose-600"}`}>
                      Joint Evaluator Name
                    </label>
                    <input 
                      type="text" 
                      value={coverData.teacher2Name || ''}
                      onChange={(e) => updateField("teacher2Name", e.target.value)}
                      placeholder="Dr. Sadia Islam Mou"
                      className={`px-3 py-2 text-[12px] font-semibold ${inputClass}`}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => updateField("teacher2Designation", e.target.value)}
                        placeholder="Associate Professor"
                        className={`px-3 py-2 text-[12px] font-medium ${inputClass}`}
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => updateField("teacher2Discipline", e.target.value)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        onChange={(e) => updateField("teacher2University", e.target.value)}
                    ...
                    <div className="space-y-1">
                      ...
                      <input 
                        type="text" 
                        value={coverData.teacher2Location || ''}
                        onChange={(e) => updateField("teacher2Location", e.target.value)}
