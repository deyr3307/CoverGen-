...
import { BookOpen, GraduationCap, FileText, Sliders, Calendar, Sparkles, ArrowRight, UserCheck, Plus, Trash2 } from 'lucide-react';
...
export function InformationForm({ 
  ...
  // 1. State Initialization: Dedicated boolean state for the Joint Evaluator toggle
  ...
  // Synchronize internal toggle state when parent coverData updates
  useEffect(() => {
  ...
  // 2. Event Handler: Flip boolean state and synchronize with parent state
    ...
    setIsJointEvaluator((prev) => {
      ...
      if (nextVal) {
        setCoverData(curr => ({
          ...
          teacher2Name: curr.teacher2Name?.trim() ? curr.teacher2Name : 'Sadia Islam Mou',
          teacher2Designation: curr.teacher2Designation?.trim() ? curr.teacher2Designation : 'Assistant Professor',
          teacher2Discipline: curr.teacher2Discipline?.trim() ? curr.teacher2Discipline : (curr.teacherDiscipline || 'Environmental Science Discipline'),
          teacher2University: curr.teacher2University?.trim() ? curr.teacher2University : (curr.teacherUniversity || 'Khulna University'),
          teacher2Location: curr.teacher2Location?.trim() ? curr.teacher2Location : (curr.teacherLocation || 'Khulna')
        }));
      } else {
        setCoverData(curr => ({
          ...curr,
    ...
    <div className={`flex flex-col h-full overflow-hidden relative transition-colors duration-300 ${
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 relative z-10">
        ...
        <div className={`p-5 rounded-2xl space-y-4 ${cardClass}`}>
            ...
            <div className="flex items-center space-x-2.5">
              <input 
                ...
                className={`w-4 h-4 rounded focus:outline-none focus:ring-0 transition-all cursor-pointer ${
                  isDark ? 'border-slate-800 bg-[#070b13] text-indigo-500' : 'border-slate-300 bg-white text-indigo-600'
                }`}
              />
              <label 
                htmlFor="joint-evaluator-toggle"
              ...
              >
                Add Joint Evaluator / Second Teacher
              </label>
            </div>

            <button
              type="button"
              id="btn-toggle-joint-evaluator"
          ...
          {/* Conditional Rendering of Second Teacher Fields */}
          {isJointEvaluator && (
            <div className={`space-y-4 pt-3 border-t border-dashed animate-fadeIn ${
              isDark ? 'border-indigo-500/30 bg-indigo-950/10 p-3.5 rounded-xl' : 'border-indigo-200 bg-indigo-50/40 p-3.5 rounded-xl'
            }`}>
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-extrabold uppercase tracking-wider text-indigo-500 dark:text-indigo-400 flex items-center gap-1.5">
                  <UserCheck className="w-3.5 h-3.5 text-indigo-500" />
                  Second Teacher / Joint Evaluator Info
                </span>
                <button
                  type="button"
                  onClick={() => handleToggleJointEvaluator()}
                  className="text-[10px] font-mono text-rose-500 hover:text-rose-600 underline cursor-pointer flex items-center gap-1"
                >
              ...
              <div>
                <label className={`block text-[10px] font-mono font-extrabold mb-1.5 uppercase ${labelClass}`}>Teacher 2 Location</label>
                <input 
                  type="text" 
                  value={coverData.teacher2Location || ''}
                  onChange={(e) => setCoverData(prev => ({ ...prev, teacher2Location: e.target.value }))}
                  placeholder="Khulna-9208, Bangladesh"
                  className={`w-full border text-xs rounded-xl px-3.5 py-2.5 outline-none transition-all font-sans ${inputClass}`}
                />
              </div>
